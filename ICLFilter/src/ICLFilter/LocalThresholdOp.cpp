    ...
    void apply_local_threshold_six(const Img<S> &src,const Img<I> &ii, ImgBase *dst, float tf, int m, float gs){
      ...
      switch(dst->getDepth()){
        ...
        case depth16u:
          if(gs!=0.0f){
            for(int c=0;c<src.getChannels();++c){
              fast_lt<S,I,icl16u,typename ThreshType<S>::T,true>(psrc,pii,dst->asImg<icl16u>()->begin(0),w,h,m,t,gs,c);
            }
          }else{
            for(int c=0;c<src.getChannels();++c){
              fast_lt<S,I,icl16u,typename ThreshType<S>::T,false>(psrc,pii,dst->asImg<icl16u>()->begin(0),w,h,m,t,gs,c);
            }
          }
          break;
        case depth32s:
