  ...
  namespace qt{
     ...
     const ImageStatistics &updateStats() const {
          ...
          for(int y=0;y<data.getHeight();++y){
            ...
            if(imageDepth == depth8u){
              ...
              histo_interleaved<icl16s>(t.data.data(),t.size.getDim(),
                                        imageChannels, grs,
                                        stats.histos);
            }else if(imageDepth == depth16u){
              histo_interleaved<icl16u>(t.data.data(),t.size.getDim(),
                                        imageChannels, grs,
                                        stats.histos);
            }else{
      ...
      std::vector<Range64f> findMinMaxGeneric(Range64f &global) const{
        ...
        if(imageDepth == depth8u){
          ...
          switch(c){
            case 1: return findMinMax<icl16u,1>(global);
            case 2: return findMinMax<icl16u,2>(global);
            case 3: return findMinMax<icl16u,3>(global);
            case 4: return findMinMax<icl16u,4>(global);
          }
        }else{
          switch(c){
      ...
      std::vector<double> findColorGeneric(int x, int y) const{
        ...
        else if(imageDepth == depth16u) return findColor<icl16u>(x,y);
      ...
      const ImgBase &extractImageGeneric() const{
        ...
        else if(imageDepth == depth16u) return extractImage<icl16u>(*extractedImageBuffer->as16u());
      ...
      void setupPixelTransfer(){
        ...
        if( (bci[0] < 0)  && (bci[1] < 0) && (bci[2] < 0)){
          ...
          switch (imageDepth){
            ...
            case depth16u:{
              static const icl64f _max = 65536;
              fScaleRGB  = l ? _max/l : _max;
              fBiasRGB = (- fScaleRGB * all.minVal)/255.0;
              break;
            }
          ...
          else if(imageDepth == depth16u) fScaleRGB = 255;
    ...
    void GLImg::update(const ImgBase *src, int maxCellSize){
      ...
      switch(src->getDepth()){
        ...
        case depth16u: m_data->bufferTextureData<icl16u, icl16u>(*src->as16u(), maxCellSize); break;
