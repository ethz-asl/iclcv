  ...
  namespace core{
    ...
    ImgBase *imgNew(depth d, const ImgParams &params){
      ...
      switch (d){
        ...
        case depth16u: return new Img16u(params); break;
...
// --- from icl16u to icl32s ---
template<> void convert<icl16u,icl32s>(const icl16u *poSrcStart,const icl16u *poSrcEnd, icl32s *poDst) {
  // Cast the first unaligned values.
  for(; (((uintptr_t)poDst) & 15) && (poSrcStart < poSrcEnd); ++poSrcStart, ++poDst) {
    *poDst = static_cast<icl32f>(*poSrcStart);
  }
  ...
  for (; poSrcStart < poSrcEnd-7; poSrcStart += 8, poDst += 8) {
    ...
    // TODO(ff): Check if this, and the instances below have to be changed for
    //           unsigned types.
    // Convert to 32s values.
    __m128i vl = _mm_unpacklo_epi16(v, vk0);
    __m128i vh = _mm_unpackhi_epi16(v, vk0);
...
// --- from icl16u to icl32f ---
template<> void convert<icl16u,icl32f>(const icl16u *poSrcStart,const icl16u *poSrcEnd, icl32f *poDst) {
  // cast the first unaligned values
  for(; (((uintptr_t)poDst) & 15) && (poSrcStart < poSrcEnd); ++poSrcStart, ++poDst) {
    *poDst = *poSrcStart < -std::numeric_limits<icl32f>::max() ? -std::numeric_limits<icl32f>::max() :
    *poSrcStart > std::numeric_limits<icl32f>::max() ? std::numeric_limits<icl32f>::max() :
...
// --- from icl16u to icl64f ---
template<> void convert<icl16u,icl64f>(const icl16u *poSrcStart,const icl16u *poSrcEnd, icl64f *poDst) {
  // Cast the first unaligned values.
  for(; (((uintptr_t)poDst) & 15) && (poSrcStart < poSrcEnd); ++poSrcStart, ++poDst) {
    *poDst = *poSrcStart < -std::numeric_limits<icl64f>::max() ? -std::numeric_limits<icl64f>::max() :
    *poSrcStart > std::numeric_limits<icl64f>::max() ? std::numeric_limits<icl64f>::max() :
...
// --- from icl32s to icl16u ---
template<> void convert<icl32s,icl16u>(const icl32s *poSrcStart,const icl32s *poSrcEnd, icl16u *poDst) {
  // Cast the first unaligned values.
  for(; (((uintptr_t)poSrcStart) & 15) && (poSrcStart < poSrcEnd); ++poSrcStart, ++poDst) {
    *poDst = *poSrcStart < std::numeric_limits<icl16u>::min() ? std::numeric_limits<icl16u>::min() :
    *poSrcStart > std::numeric_limits<icl16u>::max() ? std::numeric_limits<icl16u>::max() :
    static_cast<icl16u>(*poSrcStart);
  }
  ...
  for (; poSrcStart < poSrcEnd-7; poSrcStart += 8, poDst += 8) {
    ...
    // Convert to 16u values; store 16u values.
    _mm_storeu_si128((__m128i*)poDst, _mm_packs_epi32(v1, v2));
  }
  ...
  namespace{
      ...
      template<> double channel_mean<icl16u>(const Img16u &image, int channel, bool roiOnly){
        icl64f m=0;
        ippiMean_16u_C1R(roiOnly ? image.getROIData(channel) : image.getData(channel),image.getLineStep(),
                         roiOnly ? image.getROISize() : image.getROISize(),&m);
        return m;
      }
  ...
  #define COMPUTE_DEFAULT_HISTO_256_TEMPLATE(D)                                                                                \
      ...
      COMPUTE_DEFAULT_HISTO_256_TEMPLATE(16u)
  ...
  #define COMPUTE_COMPLEX_HISTO_TEMPLATE(D)                                                                                    \
      ...
      COMPUTE_COMPLEX_HISTO_TEMPLATE(16u)
