...
namespace icl {
  namespace core{
   ...
   /// Conversion function to transform a pointer into an object
    ...
    /// IPP-OPTIMIZED specialization for icl16u clearing (using ippiSet)
    template <>
    inline void clearChannelROI(Img<icl16u> *im, int c, icl16u clearVal, const utils::Point &offs,
                                const utils::Size &size){
      FUNCTION_LOG("");
      ICLASSERT_RETURN( im );
      ippiSet_16u_C1R(clearVal,im->getROIData(c,offs),im->getLineStep(),size);
    }
